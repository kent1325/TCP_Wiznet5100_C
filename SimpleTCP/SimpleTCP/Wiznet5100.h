...
// (Ikke ret pænt, men da den skal bruges i alle filer der inkluderer denne .h fil, er den placeret her...)
#define MAX_BUF 128

//Funktions prototypning, så de kan bruges i alle de .c filer der inkluderer vores .h til.
extern void WizInit(void);
extern uint8_t getSocketState(void);
extern uint16_t recieve(uint8_t *buf, uint16_t bufLen);
extern uint16_t send(const uint8_t *buf, uint16_t bufLen);
extern uint8_t openSocket(uint8_t eth_protocol, uint16_t tcp_port);
...
// Wiznet W5100 Register Addresser
...
#define S0_DEST_IP 0x040C	   // Socket 0: Destination IP
#define S0_DEST_Port 0x0410	   // Socket 0: Destination Port

// TX/RX Buffer register start
